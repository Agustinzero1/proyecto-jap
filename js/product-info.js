...
/** - borra la imagen placeholder y selecciona la primera por defecto
 * @param {[string]} images - lista de imagenes a agregar, puede ser una lista de URL como texto
*/
function gal_inicialize( images ){
    for (const image of images) {
        gal_element = gal_add_image(image, document.getElementsByClassName("gal-img")[0])
        gal_element.addEventListener('mouseenter', function(e){ imagen_seleccionada = gal_select_img(e) });
    }

    //elimino el placeholder
    document.getElementsByClassName("gal-tumb")[0].remove()

    //seteo la imagen por defecto en la variable global y en la seleccion
    defaultImage = document.getElementsByClassName("gal-tumb")[0]
    defaultImage.classList.add("selected-elem");
    big_image.children[0].src = defaultImage.children[0].src;
    imagen_seleccionada =  defaultImage;
}

/** - agrega una imagen a la galeria
* @param {HTMLElement} gallery - elemento HTML que corresponde a la galeria (class="gal-img")
* @param {string} image - imagen a agregar, puede ser una imagen URL como texto
*/
function gal_add_image(image, gallery) {
    
    let imagen = document.createElement("div");
    imagen.classList.add("gal-tumb")
    imagen.innerHTML =`
        <img class="gal-img-elem" src="${image}">
    `;

    gallery.getElementsByClassName("gal-list")[0].appendChild(imagen);

    return imagen;
}

function  gal_select_img(evento){
    
    if(imagen_seleccionada != undefined){
        imagen_seleccionada.classList.remove("selected-elem");
    }

    evento.target.classList.add("selected-elem");
    big_image.children[0].src = evento.target.children[0].src;
...
function Draw_ProductInfo( productInfoOBJ, comment_list) {
    ...
    //seteando la galeria
    gal_inicialize( productInfo.images );
